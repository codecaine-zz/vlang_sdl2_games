...
fn prod_fx_draw_particles(renderer &sdl.Renderer, w int, h int, t f64) {
	...
	for i in 0 .. 20 {
		...
		speed := 0.08 + frac * 0.12
		phase := (frac + t * speed)
		py := int((phase - math.floor(phase)) * f64(h))
		px := int((0.5 + 0.5 * math.sin(fi * 2.31 + t * 0.18)) * f64(w))
		glow := u8(50 + int(40.0 * (0.5 + 0.5 * math.sin(t * 1.2 + fi))))
		size := 2 + (i % 3)
		sdl.set_render_draw_color(renderer, glow + 40, glow, 30, 50)
...
fn prod_fx_draw_vignette(renderer &sdl.Renderer, w int, h int) {
	...
	for i in 0 .. 8 {
		inset := i * 4
		sdl.set_render_draw_color(renderer, 0, 0, 0, u8(7))
