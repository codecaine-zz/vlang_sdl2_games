#!/usr/bin/env python3
"""Generate 124 BPM chill board game jazz / lo-fi soundtrack for Connect 4."""
...
def generate_soundtrack() -> bytes:
    ...
    # Chill Neo-Soul / Jazz progression: Fmaj9 - Em7 - Dm9 - Cmaj9
    chords = [
        [53, 57, 60, 64, 67],  # Fmaj9
        [52, 55, 59, 62, 65],  # Em7(9)
        [50, 53, 57, 60, 64],  # Dm9
    ...
    for b in range(total_beats):
        ...
        for note in chord:
            ...
            for s in range(int(beat_samples * 0.9)):
                ...
                sine2 = math.sin(2.0 * math.pi * (freq * 2.0) * t) * 0.25
                samples[idx] += (sine + sine2) * env * 0.16

        # Warm Vibraphone Melodic Accent (on 2nd & 4th beats)
        if b % 2 == 1:
            vn = chord[(b * 2) % len(chord)] + 12
            vfreq = midi_to_freq(vn)
            for s in range(int(beat_samples * 0.8)):
                ...
                t = s / SAMPLE_RATE
                env = math.exp(-t * 6.0)
                vibe = math.sin(2.0 * math.pi * vfreq * t) + math.sin(2.0 * math.pi * (vfreq * 3.0) * t) * 0.2
                samples[idx] += vibe * env * 0.2
...
def main() -> None:
    ...
    out_path = out_dir / "connect4_bgm.wav"
    ...
    local_path = root / "connect4" / "assets" / "music" / "connect4_bgm.wav"
