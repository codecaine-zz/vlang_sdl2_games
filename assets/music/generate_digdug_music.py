#!/usr/bin/env python3
"""Generate 132 BPM arcade digging soundtrack for Dig Dug."""
...
def generate_soundtrack() -> bytes:
    ...
    # Dig Dug iconic cheerful arcade melody in F Major
    melody_notes = [
        65, 69, 72, 77, 72, 69, 65, 60,
        65, 69, 72, 77, 76, 72, 67, 60,
    ...
    for i in range(num_steps):
        ...
        for s in range(dur_samples):
            ...
            env = math.exp(-t * 12.0)
            phase = (t * freq) % 1.0
            pulse = (1.0 if phase < 0.2 else -0.25) * 0.35
            samples[idx] += pulse * env * 0.45

        # Bouncy bassline
        if i % 2 == 0:
            bn = melody_notes[i % len(melody_notes)] - 24
...
def main() -> None:
    ...
    out_path = out_dir / "digdug_bgm.wav"
    ...
    local_path = root / "digdug" / "assets" / "music" / "digdug_bgm.wav"
