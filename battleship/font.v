module main

import math
import sdl

pub struct Color {
pub:
	r u8
	g u8
	b u8
	a u8 = 255
}

// 8x8 bitmap font table
const font_8x8 = [
	// 32: Space
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 33: !
	0x18, 0x3C, 0x3C, 0x18, 0x18, 0x00, 0x18, 0x00,
	// 34: "
	0x66, 0x66, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 35: #
	0x6C, 0x6C, 0xFE, 0x6C, 0xFE, 0x6C, 0x6C, 0x00,
	// 36: $
	0x18, 0x3E, 0x60, 0x3C, 0x06, 0x7C, 0x18, 0x00,
	// 37: %
	0x00, 0xC6, 0xCC, 0x18, 0x30, 0x66, 0xC6, 0x00,
	// 38: &
	0x38, 0x6C, 0x38, 0x76, 0xDC, 0xCC, 0x76, 0x00,
	// 39: '
	0x18, 0x18, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 40: (
	0x0C, 0x18, 0x30, 0x30, 0x30, 0x18, 0x0C, 0x00,
	// 41: )
	0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x18, 0x30, 0x00,
	// 42: *
	0x00, 0x66, 0x3C, 0xFF, 0x3C, 0x66, 0x00, 0x00,
	// 43: +
	0x00, 0x18, 0x18, 0x7E, 0x18, 0x18, 0x00, 0x00,
	// 44: ,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x30,
	// 45: -
	0x00, 0x00, 0x00, 0x7E, 0x00, 0x00, 0x00, 0x00,
	// 46: .
	0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x00,
	// 47: /
	0x00, 0x06, 0x0C, 0x18, 0x30, 0x60, 0x40, 0x00,
	// 48: 0
	0x3C, 0x66, 0x6E, 0x76, 0x66, 0x66, 0x3C, 0x00,
	// 49: 1
	0x18, 0x38, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00,
	// 50: 2
	0x3C, 0x66, 0x06, 0x0C, 0x18, 0x30, 0x7E, 0x00,
	// 51: 3
	0x3C, 0x66, 0x06, 0x1C, 0x06, 0x66, 0x3C, 0x00,
	// 52: 4
	0x0C, 0x1C, 0x3C, 0x6C, 0xFE, 0x0C, 0x0C, 0x00,
	// 53: 5
	0x7E, 0x60, 0x7C, 0x06, 0x06, 0x66, 0x3C, 0x00,
	// 54: 6
	0x3C, 0x66, 0x60, 0x7C, 0x66, 0x66, 0x3C, 0x00,
	// 55: 7
	0x7E, 0x66, 0x0C, 0x18, 0x18, 0x18, 0x18, 0x00,
	// 56: 8
	0x3C, 0x66, 0x66, 0x3C, 0x66, 0x66, 0x3C, 0x00,
	// 57: 9
	0x3C, 0x66, 0x66, 0x3E, 0x06, 0x66, 0x3C, 0x00,
	// 58: :
	0x00, 0x18, 0x18, 0x00, 0x18, 0x18, 0x00, 0x00,
	// 59: ;
	0x00, 0x18, 0x18, 0x00, 0x18, 0x18, 0x30, 0x00,
	// 60: <
	0x06, 0x0C, 0x18, 0x30, 0x18, 0x0C, 0x06, 0x00,
	// 61: =
	0x00, 0x7E, 0x00, 0x00, 0x7E, 0x00, 0x00, 0x00,
	// 62: >
	0x60, 0x30, 0x18, 0x0C, 0x18, 0x30, 0x60, 0x00,
	// 63: ?
	0x3C, 0x66, 0x06, 0x0C, 0x18, 0x00, 0x18, 0x00,
	// 64: @
	0x3C, 0x66, 0x6E, 0x6A, 0x6E, 0x60, 0x3C, 0x00,
	// 65: A
	0x18, 0x3C, 0x66, 0x66, 0x7E, 0x66, 0x66, 0x00,
	// 66: B
	0x7C, 0x66, 0x66, 0x7C, 0x66, 0x66, 0x7C, 0x00,
	// 67: C
	0x3C, 0x66, 0x60, 0x60, 0x60, 0x66, 0x3C, 0x00,
	// 68: D
	0x78, 0x6C, 0x66, 0x66, 0x66, 0x6C, 0x78, 0x00,
	// 69: E
	0x7E, 0x60, 0x60, 0x7C, 0x60, 0x60, 0x7E, 0x00,
	// 70: F
	0x7E, 0x60, 0x60, 0x7C, 0x60, 0x60, 0x60, 0x00,
	// 71: G
	0x3C, 0x66, 0x60, 0x6E, 0x66, 0x66, 0x3C, 0x00,
	// 72: H
	0x66, 0x66, 0x66, 0x7E, 0x66, 0x66, 0x66, 0x00,
	// 73: I
	0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00,
	// 74: J
	0x06, 0x06, 0x06, 0x06, 0x66, 0x66, 0x3C, 0x00,
	// 75: K
	0x66, 0x6C, 0x78, 0x70, 0x78, 0x6C, 0x66, 0x00,
	// 76: L
	0x60, 0x60, 0x60, 0x60, 0x60, 0x60, 0x7E, 0x00,
	// 77: M
	0x63, 0x77, 0x7F, 0x6B, 0x63, 0x63, 0x63, 0x00,
	// 78: N
	0x66, 0x76, 0x7E, 0x7E, 0x6E, 0x66, 0x66, 0x00,
	// 79: O
	0x3C, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00,
	// 80: P
	0x7C, 0x66, 0x66, 0x7C, 0x60, 0x60, 0x60, 0x00,
	// 81: Q
	0x3C, 0x66, 0x66, 0x66, 0x6A, 0x6C, 0x36, 0x00,
	// 82: R
	0x7C, 0x66, 0x66, 0x7C, 0x6C, 0x66, 0x66, 0x00,
	// 83: S
	0x3C, 0x66, 0x60, 0x3C, 0x06, 0x66, 0x3C, 0x00,
	// 84: T
	0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x00,
	// 85: U
	0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00,
	// 86: V
	0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x00,
	// 87: W
	0x63, 0x63, 0x63, 0x6B, 0x7F, 0x77, 0x63, 0x00,
	// 88: X
	0x66, 0x66, 0x3C, 0x18, 0x3C, 0x66, 0x66, 0x00,
	// 89: Y
	0x66, 0x66, 0x66, 0x3C, 0x18, 0x18, 0x18, 0x00,
	// 90: Z
	0x7E, 0x06, 0x0C, 0x18, 0x30, 0x60, 0x7E, 0x00,
	// 91: [
	0x3C, 0x30, 0x30, 0x30, 0x30, 0x30, 0x3C, 0x00,
	// 92: \
	0x00, 0x60, 0x30, 0x18, 0x0C, 0x06, 0x02, 0x00,
	// 93: ]
	0x3C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x3C, 0x00,
]

pub fn draw_char(renderer &sdl.Renderer, x int, y int, ch u8, scale int, col Color) {
	if ch < 32 || ch > 93 {
		return
	}
	idx := (int(ch) - 32) * 8
	sdl.set_render_draw_color(renderer, col.r, col.g, col.b, col.a)

	for row in 0 .. 8 {
		bits := font_8x8[idx + row]
		for col_i in 0 .. 8 {
			if (bits & (1 << (7 - col_i))) != 0 {
				rect := sdl.Rect{
					x: x + col_i * scale
					y: y + row * scale
					w: scale
					h: scale
				}
				sdl.render_fill_rect(renderer, &rect)
			}
		}
	}
}

pub fn draw_text(renderer &sdl.Renderer, x int, y int, text string, scale int, col Color) {
	for i, ch in text {
		mut c := ch
		if c >= `a` && c <= `z` {
			c = c - 32
		}
		draw_char(renderer, x + i * 8 * scale, y, c, scale, col)
	}
}

pub fn draw_text_centered(renderer &sdl.Renderer, cx int, y int, text string, scale int, col Color) {
	w := text.len * 8 * scale
	draw_text(renderer, cx - w / 2, y, text, scale, col)
}

pub fn draw_filled_circle(renderer &sdl.Renderer, cx int, cy int, r int, col Color) {
	sdl.set_render_draw_color(renderer, col.r, col.g, col.b, col.a)
	for dy := -r; dy <= r; dy++ {
		dx_limit := int(math.sqrt(f64(r * r - dy * dy)))
		line := sdl.Rect{
			x: cx - dx_limit
			y: cy + dy
			w: dx_limit * 2 + 1
			h: 1
		}
		sdl.render_fill_rect(renderer, &line)
	}
}

pub fn draw_circle_outline(renderer &sdl.Renderer, cx int, cy int, r int, col Color) {
	sdl.set_render_draw_color(renderer, col.r, col.g, col.b, col.a)
	mut x := r
	mut y := 0
	mut err := 0

	for x >= y {
		sdl.render_draw_point(renderer, cx + x, cy + y)
		sdl.render_draw_point(renderer, cx + y, cy + x)
		sdl.render_draw_point(renderer, cx - y, cy + x)
		sdl.render_draw_point(renderer, cx - x, cy + y)
		sdl.render_draw_point(renderer, cx - x, cy - y)
		sdl.render_draw_point(renderer, cx - y, cy - x)
		sdl.render_draw_point(renderer, cx + y, cy - x)
		sdl.render_draw_point(renderer, cx + x, cy - y)

		if err <= 0 {
			y += 1
			err += 2 * y + 1
		}
		if err > 0 {
			x -= 1
			err -= 2 * x + 1
		}
	}
}

pub fn draw_beveled_box(renderer &sdl.Renderer, x int, y int, w int, h int, bg Color, border_light Color, border_dark Color) {
	sdl.set_render_draw_color(renderer, bg.r, bg.g, bg.b, bg.a)
	box := sdl.Rect{x: x, y: y, w: w, h: h}
	sdl.render_fill_rect(renderer, &box)

	sdl.set_render_draw_color(renderer, border_light.r, border_light.g, border_light.b, border_light.a)
	sdl.render_draw_line(renderer, x, y, x + w - 1, y)
	sdl.render_draw_line(renderer, x, y, x, y + h - 1)

	sdl.set_render_draw_color(renderer, border_dark.r, border_dark.g, border_dark.b, border_dark.a)
	sdl.render_draw_line(renderer, x, y + h - 1, x + w - 1, y + h - 1)
	sdl.render_draw_line(renderer, x + w - 1, y, x + w - 1, y + h - 1)
}
