module main

import sdl

const font_8x8 = [
	// 32: Space
	u8(0x00), 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 33: !
	0x00, 0x00, 0x5F, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 34: "
	0x00, 0x07, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00,
	// 35: #
	0x14, 0x7F, 0x14, 0x7F, 0x14, 0x00, 0x00, 0x00,
	// 36: $
	0x24, 0x2A, 0x7F, 0x2A, 0x12, 0x00, 0x00, 0x00,
	// 37: %
	0x23, 0x13, 0x08, 0x64, 0x62, 0x00, 0x00, 0x00,
	// 38: &
	0x36, 0x49, 0x55, 0x22, 0x50, 0x00, 0x00, 0x00,
	// 39: '
	0x00, 0x05, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 40: (
	0x00, 0x1C, 0x22, 0x41, 0x00, 0x00, 0x00, 0x00,
	// 41: )
	0x00, 0x41, 0x22, 0x1C, 0x00, 0x00, 0x00, 0x00,
	// 42: *
	0x14, 0x08, 0x3E, 0x08, 0x14, 0x00, 0x00, 0x00,
	// 43: +
	0x08, 0x08, 0x3E, 0x08, 0x08, 0x00, 0x00, 0x00,
	// 44: ,
	0x00, 0x50, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 45: -
	0x08, 0x08, 0x08, 0x08, 0x08, 0x00, 0x00, 0x00,
	// 46: .
	0x00, 0x60, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 47: /
	0x20, 0x10, 0x08, 0x04, 0x02, 0x00, 0x00, 0x00,
	// 48: 0
	0x3E, 0x51, 0x49, 0x45, 0x3E, 0x00, 0x00, 0x00,
	// 49: 1
	0x00, 0x42, 0x7F, 0x40, 0x00, 0x00, 0x00, 0x00,
	// 50: 2
	0x42, 0x61, 0x51, 0x49, 0x46, 0x00, 0x00, 0x00,
	// 51: 3
	0x21, 0x41, 0x45, 0x4B, 0x31, 0x00, 0x00, 0x00,
	// 52: 4
	0x18, 0x14, 0x12, 0x7F, 0x10, 0x00, 0x00, 0x00,
	// 53: 5
	0x27, 0x45, 0x45, 0x45, 0x39, 0x00, 0x00, 0x00,
	// 54: 6
	0x3C, 0x4A, 0x49, 0x49, 0x30, 0x00, 0x00, 0x00,
	// 55: 7
	0x01, 0x71, 0x09, 0x05, 0x03, 0x00, 0x00, 0x00,
	// 56: 8
	0x36, 0x49, 0x49, 0x49, 0x36, 0x00, 0x00, 0x00,
	// 57: 9
	0x06, 0x49, 0x49, 0x29, 0x1E, 0x00, 0x00, 0x00,
	// 58: :
	0x00, 0x36, 0x36, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 59: ;
	0x00, 0x56, 0x36, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 60: <
	0x08, 0x14, 0x22, 0x41, 0x00, 0x00, 0x00, 0x00,
	// 61: =
	0x14, 0x14, 0x14, 0x14, 0x14, 0x00, 0x00, 0x00,
	// 62: >
	0x41, 0x22, 0x14, 0x08, 0x00, 0x00, 0x00, 0x00,
	// 63: ?
	0x02, 0x01, 0x51, 0x09, 0x06, 0x00, 0x00, 0x00,
	// 64: @
	0x32, 0x49, 0x79, 0x41, 0x3E, 0x00, 0x00, 0x00,
	// 65: A
	0x7E, 0x11, 0x11, 0x11, 0x7E, 0x00, 0x00, 0x00,
	// 66: B
	0x7F, 0x49, 0x49, 0x49, 0x36, 0x00, 0x00, 0x00,
	// 67: C
	0x3E, 0x41, 0x41, 0x41, 0x22, 0x00, 0x00, 0x00,
	// 68: D
	0x7F, 0x41, 0x41, 0x22, 0x1C, 0x00, 0x00, 0x00,
	// 69: E
	0x7F, 0x49, 0x49, 0x49, 0x41, 0x00, 0x00, 0x00,
	// 70: F
	0x7F, 0x09, 0x09, 0x09, 0x01, 0x00, 0x00, 0x00,
	// 71: G
	0x3E, 0x41, 0x49, 0x49, 0x7A, 0x00, 0x00, 0x00,
	// 72: H
	0x7F, 0x08, 0x08, 0x08, 0x7F, 0x00, 0x00, 0x00,
	// 73: I
	0x00, 0x41, 0x7F, 0x41, 0x00, 0x00, 0x00, 0x00,
	// 74: J
	0x20, 0x40, 0x41, 0x3F, 0x01, 0x00, 0x00, 0x00,
	// 75: K
	0x7F, 0x08, 0x14, 0x22, 0x41, 0x00, 0x00, 0x00,
	// 76: L
	0x7F, 0x40, 0x40, 0x40, 0x40, 0x00, 0x00, 0x00,
	// 77: M
	0x7F, 0x02, 0x0C, 0x02, 0x7F, 0x00, 0x00, 0x00,
	// 78: N
	0x7F, 0x04, 0x08, 0x10, 0x7F, 0x00, 0x00, 0x00,
	// 79: O
	0x3E, 0x41, 0x41, 0x41, 0x3E, 0x00, 0x00, 0x00,
	// 80: P
	0x7F, 0x09, 0x09, 0x09, 0x06, 0x00, 0x00, 0x00,
	// 81: Q
	0x3E, 0x41, 0x51, 0x21, 0x5E, 0x00, 0x00, 0x00,
	// 82: R
	0x7F, 0x09, 0x19, 0x29, 0x46, 0x00, 0x00, 0x00,
	// 83: S
	0x46, 0x49, 0x49, 0x49, 0x31, 0x00, 0x00, 0x00,
	// 84: T
	0x01, 0x01, 0x7F, 0x01, 0x01, 0x00, 0x00, 0x00,
	// 85: U
	0x3F, 0x40, 0x40, 0x40, 0x3F, 0x00, 0x00, 0x00,
	// 86: V
	0x1F, 0x20, 0x40, 0x20, 0x1F, 0x00, 0x00, 0x00,
	// 87: W
	0x3F, 0x40, 0x38, 0x40, 0x3F, 0x00, 0x00, 0x00,
	// 88: X
	0x63, 0x14, 0x08, 0x14, 0x63, 0x00, 0x00, 0x00,
	// 89: Y
	0x07, 0x08, 0x70, 0x08, 0x07, 0x00, 0x00, 0x00,
	// 90: Z
	0x61, 0x51, 0x49, 0x45, 0x43, 0x00, 0x00, 0x00,
	// 91: [
	0x00, 0x7F, 0x41, 0x41, 0x00, 0x00, 0x00, 0x00,
	// 92: \
	0x02, 0x04, 0x08, 0x10, 0x20, 0x00, 0x00, 0x00,
	// 93: ]
	0x00, 0x41, 0x41, 0x7F, 0x00, 0x00, 0x00, 0x00,
	// 94: ^
	0x04, 0x02, 0x01, 0x02, 0x04, 0x00, 0x00, 0x00,
	// 95: _
	0x40, 0x40, 0x40, 0x40, 0x40, 0x00, 0x00, 0x00,
	// 96: `
	0x00, 0x01, 0x02, 0x04, 0x00, 0x00, 0x00, 0x00,
	// 97: a
	0x20, 0x54, 0x54, 0x54, 0x78, 0x00, 0x00, 0x00,
	// 98: b
	0x7F, 0x48, 0x44, 0x44, 0x38, 0x00, 0x00, 0x00,
	// 99: c
	0x38, 0x44, 0x44, 0x44, 0x20, 0x00, 0x00, 0x00,
	// 100: d
	0x38, 0x44, 0x44, 0x48, 0x7F, 0x00, 0x00, 0x00,
	// 101: e
	0x38, 0x54, 0x54, 0x54, 0x18, 0x00, 0x00, 0x00,
	// 102: f
	0x08, 0x7E, 0x09, 0x01, 0x02, 0x00, 0x00, 0x00,
	// 103: g
	0x0C, 0x52, 0x52, 0x52, 0x3E, 0x00, 0x00, 0x00,
	// 104: h
	0x7F, 0x08, 0x04, 0x04, 0x78, 0x00, 0x00, 0x00,
	// 105: i
	0x00, 0x44, 0x7D, 0x40, 0x00, 0x00, 0x00, 0x00,
	// 106: j
	0x20, 0x40, 0x44, 0x3D, 0x00, 0x00, 0x00, 0x00,
	// 107: k
	0x7F, 0x10, 0x28, 0x44, 0x00, 0x00, 0x00, 0x00,
	// 108: l
	0x00, 0x41, 0x7F, 0x40, 0x00, 0x00, 0x00, 0x00,
	// 109: m
	0x7C, 0x04, 0x18, 0x04, 0x78, 0x00, 0x00, 0x00,
	// 110: n
	0x7C, 0x08, 0x04, 0x04, 0x78, 0x00, 0x00, 0x00,
	// 111: o
	0x38, 0x44, 0x44, 0x44, 0x38, 0x00, 0x00, 0x00,
	// 112: p
	0x7C, 0x14, 0x14, 0x14, 0x08, 0x00, 0x00, 0x00,
	// 113: q
	0x08, 0x14, 0x14, 0x18, 0x7C, 0x00, 0x00, 0x00,
	// 114: r
	0x7C, 0x08, 0x04, 0x04, 0x08, 0x00, 0x00, 0x00,
	// 115: s
	0x48, 0x54, 0x54, 0x54, 0x24, 0x00, 0x00, 0x00,
	// 116: t
	0x04, 0x3E, 0x44, 0x24, 0x00, 0x00, 0x00, 0x00,
	// 117: u
	0x3C, 0x40, 0x40, 0x20, 0x7C, 0x00, 0x00, 0x00,
	// 118: v
	0x1C, 0x20, 0x40, 0x20, 0x1C, 0x00, 0x00, 0x00,
	// 119: w
	0x3C, 0x40, 0x30, 0x40, 0x3C, 0x00, 0x00, 0x00,
	// 120: x
	0x44, 0x28, 0x10, 0x28, 0x44, 0x00, 0x00, 0x00,
	// 121: y
	0x0C, 0x50, 0x50, 0x50, 0x3C, 0x00, 0x00, 0x00,
	// 122: z
	0x44, 0x64, 0x54, 0x4C, 0x44, 0x00, 0x00, 0x00,
	// 123: {
	0x00, 0x08, 0x36, 0x41, 0x00, 0x00, 0x00, 0x00,
	// 124: |
	0x00, 0x00, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00,
	// 125: }
	0x00, 0x41, 0x36, 0x08, 0x00, 0x00, 0x00, 0x00,
	// 126: ~
	0x02, 0x01, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00,
]

struct Color {
	r u8
	g u8
	b u8
	a u8 = 255
}

fn draw_char(renderer &sdl.Renderer, x int, y int, ch rune, scale int, color Color) {
	ascii_val := int(ch)
	if ascii_val < 32 || ascii_val > 126 {
		return
	}
	font_idx := (ascii_val - 32) * 8

	sdl.set_render_draw_color(renderer, color.r, color.g, color.b, color.a)

	for col_idx in 0 .. 8 {
		line_bits := font_8x8[font_idx + col_idx]
		for row_idx in 0 .. 8 {
			if (line_bits & (1 << u32(row_idx))) != 0 {
				rect := sdl.Rect{
					x: x + col_idx * scale
					y: y + row_idx * scale
					w: scale
					h: scale
				}
				sdl.render_fill_rect(renderer, &rect)
			}
		}
	}
}

fn draw_text(renderer &sdl.Renderer, x int, y int, text string, scale int, color Color) {
	mut current_x := x
	for ch in text {
		draw_char(renderer, current_x, y, ch, scale, color)
		current_x += 8 * scale
	}
}

fn draw_text_centered(renderer &sdl.Renderer, cx int, y int, text string, scale int, color Color) {
	text_len := text.len
	total_width := text_len * 8 * scale
	start_x := cx - (total_width / 2)
	draw_text(renderer, start_x, y, text, scale, color)
}

// 7-segment digital LED font renderer for classic retro counters
// Segments:
//   --- a ---
//  |         |
//  f         b
//  |         |
//   --- g ---
//  |         |
//  e         c
//  |         |
//   --- d ---
const seg_patterns = [
	u8(0x3F), // 0: a,b,c,d,e,f
	0x06,     // 1: b,c
	0x5B,     // 2: a,b,d,e,g
	0x4F,     // 3: a,b,c,d,g
	0x66,     // 4: b,c,f,g
	0x6D,     // 5: a,c,d,f,g
	0x7D,     // 6: a,c,d,e,f,g
	0x07,     // 7: a,b,c
	0x7F,     // 8: a,b,c,d,e,f,g
	0x6F,     // 9: a,b,c,d,f,g
]

fn draw_digit_7seg(renderer &sdl.Renderer, x int, y int, digit int, w int, h int, on_color Color, off_color Color) {
	thick := if w / 5 > 2 { w / 5 } else { 3 }
	gap := 1
	pattern := if digit >= 0 && digit <= 9 {
		seg_patterns[digit]
	} else if digit == -1 {
		u8(0x40) // '-' minus sign
	} else {
		u8(0x00) // blank
	}

	half_h := h / 2

	// Helper to draw segment
	draw_seg := fn (renderer &sdl.Renderer, active bool, rx int, ry int, rw int, rh int, on_c Color, off_c Color) {
		c := if active { on_c } else { off_c }
		sdl.set_render_draw_color(renderer, c.r, c.g, c.b, c.a)
		r := sdl.Rect{x: rx, y: ry, w: rw, h: rh}
		sdl.render_fill_rect(renderer, &r)
	}

	// a: Top horizontal
	draw_seg(renderer, (pattern & 0x01) != 0, x + thick + gap, y, w - 2 * (thick + gap), thick, on_color, off_color)
	// b: Top-right vertical
	draw_seg(renderer, (pattern & 0x02) != 0, x + w - thick, y + thick + gap, thick, half_h - thick - 2 * gap, on_color, off_color)
	// c: Bottom-right vertical
	draw_seg(renderer, (pattern & 0x04) != 0, x + w - thick, y + half_h + gap, thick, half_h - thick - 2 * gap, on_color, off_color)
	// d: Bottom horizontal
	draw_seg(renderer, (pattern & 0x08) != 0, x + thick + gap, y + h - thick, w - 2 * (thick + gap), thick, on_color, off_color)
	// e: Bottom-left vertical
	draw_seg(renderer, (pattern & 0x10) != 0, x, y + half_h + gap, thick, half_h - thick - 2 * gap, on_color, off_color)
	// f: Top-left vertical
	draw_seg(renderer, (pattern & 0x20) != 0, x, y + thick + gap, thick, half_h - thick - 2 * gap, on_color, off_color)
	// g: Middle horizontal
	draw_seg(renderer, (pattern & 0x40) != 0, x + thick + gap, y + half_h - thick / 2, w - 2 * (thick + gap), thick, on_color, off_color)
}

fn draw_number_7seg(renderer &sdl.Renderer, x int, y int, val int, num_digits int, dw int, dh int, spacing int, on_c Color, off_c Color) {
	is_neg := val < 0
	mut abs_val := if val < 0 { -val } else { val }
	if abs_val > 999 {
		abs_val = 999
	}

	mut digits := []int{len: num_digits, init: 0}
	mut temp := abs_val
	for i := num_digits - 1; i >= 0; i-- {
		digits[i] = temp % 10
		temp /= 10
	}

	if is_neg && num_digits > 1 {
		digits[0] = -1 // minus sign on leftmost digit
	}

	for i, d in digits {
		dx := x + i * (dw + spacing)
		draw_digit_7seg(renderer, dx, y, d, dw, dh, on_c, off_c)
	}
}
