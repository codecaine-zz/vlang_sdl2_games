...
fn (sm &SoundManager) play_turn_sound() {
...
fn (sm &SoundManager) play_boost_sound() {
	...
	for i in 0 .. num_samples {
		...
		env := math.exp(-25.0 * t)
		sample := (math.sin(2.0 * math.pi * freq * t) + 0.3 * math.sin(4.0 * math.pi * freq * t)) * env * 14000.0
		pcm[i] = i16(sample)
	}
	sdl.queue_audio(sm.dev, pcm.data, u32(num_samples * 2))
}
...
fn (sm &SoundManager) play_crash_sound() {
	...
	for i in 0 .. num_samples {
		...
		env := math.exp(-5.5 * t)
		sample := (noise * 0.7 + sub * 0.3) * env * 24000.0
		pcm[i] = i16(sample)
	}
	sdl.queue_audio(sm.dev, pcm.data, u32(num_samples * 2))
}
...
fn (sm &SoundManager) play_win_sound() {
	...
	for i in 0 .. num_samples {
		...
		env := math.exp(-8.0 * t)
		sample := (math.sin(2.0 * math.pi * freq * t) + 0.4 * math.sin(4.0 * math.pi * freq * t)) * env * 16000.0
		pcm[i] = i16(sample)
	}
	sdl.queue_audio(sm.dev, pcm.data, u32(num_samples * 2))
}
