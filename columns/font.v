...
// Built-in 8x8 ASCII bitmap font for crisp UI rendering without external TTF assets.
// Covers ASCII 32 (space) to 126 (~). Each character is represented by 8 bytes (8x8 pixels).
const font_8x8 = [
	// 32: Space
...
fn draw_text_centered_fitted(renderer &sdl.Renderer, cx int, y int, text string, max_scale int, max_width int, color Color) {
	mut scale := max_scale
	for scale > 1 && text.len * 8 * scale > max_width {
		scale--
	}
	draw_text_centered(renderer, cx, y, text, scale, color)
}
...
fn (b Button) render(renderer &sdl.Renderer, mx int, my int) {
	hovered := b.is_hovered(mx, my)
	bg := if hovered { b.hover_color } else { b.bg_color }
	...
	cx := b.x + b.w / 2
	mut scale := 2
	if b.text.len * 16 > b.w - 8 {
		scale = 1
	}
	cy := b.y + (b.h - 8 * scale) / 2
	draw_text_centered(renderer, cx, cy, b.text, scale, b.text_color)
}
