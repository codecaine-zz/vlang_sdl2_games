...
pub fn draw_char(renderer &sdl.Renderer, x int, y int, ch rune, scale int, color Color) {
	ascii_val := int(ch)
...
pub fn draw_text_shadow(renderer &sdl.Renderer, x int, y int, text string, scale int, color Color, shadow Color) {
	draw_text(renderer, x + scale, y + scale, text, scale, shadow)
	draw_text(renderer, x, y, text, scale, color)
}

pub fn draw_text_centered(renderer &sdl.Renderer, cx int, y int, text string, scale int, color Color) {
	text_len := text.len
...
pub fn draw_text_centered_shadow(renderer &sdl.Renderer, cx int, y int, text string, scale int, color Color, shadow Color) {
	text_len := text.len
	total_width := text_len * 8 * scale
	start_x := cx - (total_width / 2)
	draw_text_shadow(renderer, start_x, y, text, scale, color, shadow)
}
